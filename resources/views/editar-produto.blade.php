            ...
            <form method="POST" action="{{ route('produtos.update', $product) }}" enctype="multipart/form-data">
                ...
                <div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg overflow-hidden md:flex flex-grow">
                    ...
                    <div class="md:w-1/2 p-6 flex-shrink-0 flex flex-col items-center justify-center gap-4">
                        <label for="image" class="block text-lg font-semibold text-gray-700 dark:text-gray-300 self-start">Imagem do Produto</label>
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Deixe em branco para manter a imagem atual.</p>
                        </div>
                        @error('image')
                    ...
                    <div class="md:w-1/2 p-6 flex flex-col text-gray-900 dark:text-gray-100">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm text-2xl font-bold dark:bg-gray-700 dark:text-gray-100" required>
                            @error('name') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="category_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Categoria</label>
                            <select name="category_id" id="category_id" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-100" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="5" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-100" required>{{ old('description', $product->description) }}</textarea>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" step="0.01" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-100" required>
                                @error('price') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="quantity" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Quantidade</label>
                                <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $product->quantity) }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-100" required>
                        ...
                        <div class="mt-auto flex items-center justify-end gap-4 pt-4">
                            <a href="{{ route('produtos.index') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:underline">
                                Cancelar
                            </a>
